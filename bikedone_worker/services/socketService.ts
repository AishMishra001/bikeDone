...
export const socketService = {
  ...
  sendMessage: async (jobId: string, senderId: string, text: string) => {
    ...
    try {
      await api.post(`/service-requests/${jobId}/chat`, {
        senderId,
        text,
        timestamp: Date.now()
      }, { targetService: "OMS" });
  ...
  fetchChatHistory: async (jobId: string) => {
    const { api } = require('./api');
    try {
      const response = await api.get(`/service-requests/${jobId}/chat`, { targetService: "OMS" });
      return response || [];
    } catch (e) {
  ...
  updateLocation: async (jobId: string, latitude: number, longitude: number) => {
    const { api } = require('./api');
    try {
      await api.post(`/service-requests/${jobId}/location`, {
        latitude,
        longitude
      }, { targetService: "OMS" });
    } catch (e) {
